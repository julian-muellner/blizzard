...
from executors.sink_translation import MCState
...
class AbsorbingTranslation(Executor):
    """Translate a program into a Markov chain that has absorbing states. 
    Steady state probability of terminal states is reachability probability"""
    states: Set[MCState]
    ...
    def convertProgram(self):
        ...
        self.__add_transition__(self.violation_state, self.violation_state, 1)
    ...
    def executeEnd(self, stmt: EndStatem, ctx):
        ...
        # add self-loop
        self.__add_transition__(mc_state, mc_state, 1)
        return []
    ...
    def __convert_to_prism__(self):
        ...
        prism += " " * 4 + f"pc: [-1..{self.program.max_pc}] init 0;\n"
        for v in self.program.variables:
