...
# The sink state has PC program.max_pc + 1 and all variables are else set to their lower bound
# The violation state has PC (-1) and all variables set to their lower bound
...
class ConversionExecutor(Executor):
    ...
    def __init__(self, program: Program):
        ...
        # construct sink state
        state =  {}
        for v in program.variables:
            assert(isinstance(program.get_type(v), FiniteRange))
            state[v] = program.get_type(v).lower - 1
        self.sink_state = MCState(state, program.max_pc + 1)
        self.__add_transition__(self.sink_state, self.sink_state, 1)

        # construct violation state
        self.violation_state = MCState(state, -1)
        self.__add_transition__(self.violation_state, self.sink_state, 1)

    def convertProgram(self):
        state =  {}
        stmt = self.program.statements[0]
        initial = MCState(state, stmt.pc - 1)
        self.states.add(initial)
        self.execute(stmt, (state, initial))
    ...
    def executeIf(self, stmt: IfStatem, ctx):
        ...
        if cond:
            self.execute(stmt.if_branch[0], ctx)
        else:
            self.execute(stmt.else_branch[0], ctx)

    def executeWhile(self, stmt: WhileStatem, ctx):
        state, _ = ctx
        cond = stmt.condition.evaluate(state)
        if cond:
            self.execute(stmt.body[0], ctx)
        else:
            self.execute(stmt.next_stmt, ctx)
    ...
    def executeDistAssign(self, stmt: DistAssignment, ctx):
        ...
        for (val, prob) in possible_values:
            ...
            self.execute(stmt.next_stmt, (state_copy, mc_state))
    ...
    def executePolyAssign(self, stmt: PolyAssignment, ctx):
        ...
        self.states.add(mc_state)
        self.execute(stmt.next_stmt, (state, mc_state))
